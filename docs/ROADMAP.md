...
| T072 | - [ ]   | 관심목록(찜)                                                    | MVP 이후                                                                                                                                                                                           |
| T073 | 🟢 부분 | 풀 마이페이지 대시보드(판매/구매 통계, 패널티 현황)             | MVP 이후                                                                                                                                                                                           |
...
| T077 | 🟢 부분 | MVP 이후 UX·부가 기능 개선 묶음 (2026-07-02)                    | 커스텀 OTP 회원가입·경매 진행시간 선택·판매자 상품 수정·상세 갤러리 썸네일 전환·카드 시작가/즉시구매가 표시·누적 방문 카운터·회원가입/프로필 중복 체크·인앱 브라우저 세션 대응 등 (T077 세부 참조) |
...
- **T071 세부 — 홈 상태 필터(부분 구현)**
  ...
  - [x] `AuctionStatusFilter`를 `basePath`/`defaultStatus` 주입형으로 일반화해 내 상품 페이지(`/my-products`)에서 재사용(기본값 "전체") — 2026-07-02
...
- **T077 세부 — MVP 이후 UX·부가 기능 개선 (2026-07-02) 🟢 부분**
  - [x] 회원가입: 커스텀 OTP 인증번호 방식 전환(`app/api/auth/otp/{request,verify,resend}`), 닉네임·이메일 중복 체크, 인증 후 홈 이동
  - [x] 경매 등록: 진행 시간 선택(공통코드 `auction_duration`, 12h~7일, 기본 1일) — 🟢 ISSUE-023
  - [x] 판매자 상품 관리: 내 상품 목록 페이지(`/my-products`)+하단 네비 "내 상품" 탭(로그인 게이트 `bottom-nav-gate`), 상품 정보(사진/설명/즉시구매가) 수정(`/auctions/[id]/edit`)
  - [x] 상세 이미지 갤러리: 썸네일 클릭 시 대표 이미지 전환 인터랙션(기존 Phase 3 예정분 구현)
  - [x] 홈 카드: 시작가·즉시구매가 표시(`AuctionSummary.buyNowPrice` 추가), 첫 행 대표 이미지 우선 로드(LCP 개선)
  - [x] 메인 누적 방문 카운터(`site-visit-counter`, `lib/queries/site.ts`)
  - [x] 인증 안정화: 인앱 브라우저 세션 유실 대응, 로그인/로그아웃 시 클라이언트 캐시 잔존 제거(하드 네비게이션), 프로필 닉네임 중복 저장 실패 처리, 푸터 저작권 연도 동적화
  - [ ] 판매/구매 통계·패널티 현황 대시보드(T073 잔여 범위) — 미구현(MVP 이후)
...
| 7     | MVP 이후            | T070~T077     | 8       | 1    | 보류¹   |
| —     | **MVP 합계**        | **T001~T064** | **36**  | 36   | ✅ 완료 |

> ¹ Phase 7은 보류 범위이나 일부 선반영됨: **T076 미저장 필드(description/comment) 완료**, **T075 미결정 정책 공통코드 DB 이관 대부분 완료**(레벨 산정식 제외, `- [~]`), **T071 홈 상태 필터**(커밋 `4ae51a8`)·**T073 내 상품 목록/하단 네비**·**T077 MVP 이후 UX·부가 기능 묶음**(커스텀 OTP 회원가입·진행시간 선택·상품 수정·갤러리 썸네일·카드 가격표시·방문 카운터·인증 안정화, 2026-07-02) 선반영. 완료 열은 온전히 완료된 T076만 집계(부분 선반영 🟢은 미집계).
...
| 023   | 🟢   | T077(후속)       | 경매 진행 시간 고정(36h) → 등록자 선택(공통코드 `auction_duration`, 12h~7일) — 2026-07-02 |

...
- **기능/페이지/데이터 모델**: [`PRD.md`](./PRD.md) (F001~~F012, F020~~F021)
...
- **미결정 항목 / 리스크**: [`ISSUES.md`](./ISSUES.md) (ISSUE-001~008)
...
| Task | 상태 | 작업                                                | 의존성 | 관련 기능             | 리스크                               |
| ---- | ---- | --------------------------------------------------- | ------ | --------------------- | ------------------------------------ |
| T010 | ✅   | 도메인 공용 타입 정의 (`lib/types/*`) ⭐ - 우선순위 | -      | 전 기능 / 데이터 모델 | ISSUE-001/005 (상수·산정식 자리표시) |
| T011 | ✅   | 도메인 상수 정의 (경매시간 36h 등)                  | T010   | F001, F006            | 🟡 ISSUE-001, 🔴 002/003             |
| T012 | ✅   | 전체 라우트 골격(빈 페이지) 스캐폴딩                | T002   | 전 페이지             | ISSUE-011 (cacheComponents Suspense) |
...
| T031 | ✅   | 입찰/즉시구매/등록 폼 인터랙션 (Mock)         | T023, T024 | F001, F004, F005       | 🔴 ISSUE-003   |
| T032 | ✅   | 거래 액션 인터랙션 (완료/취소/내리기/평점)    | T025       | F007, F008, F010, F011 | 🔴 004/006/007 |
| T033 | ✅   | 반응형·다크모드·접근성 + Playwright 화면 검증 | T030~T032  | 전 화면                | -              |

- **T030 세부** ✅: 공용 `empty-state`/`error-state` + 화면별 스켈레톤(경매카드/그리드·거래·메시지·프로필), 라우트 `loading.tsx` 5종 + 루트 `error.tsx`(use client+reset, SiteHeader 의존성 회피 자체완결형) + `not-found.tsx`. `/sample` `state-showcase`(Tabs 3상태 토글) 등록
- **T031 세부** ✅: `bid-panel` 입찰가 검증·낙관적 UI(현재가/입찰수 갱신)·즉시구매 확인 다이얼로그→낙찰확정, `auction-form` 필수값/즉시구매가 검증+제출 시뮬레이션, `profile-edit-form` 닉네임 검증+저장 피드백
- **T032 세부** ✅: 공용 `confirm-dialog` 신설, `transaction-card`(RSC 유지) 액션부를 `transaction-actions`(client)로 분리 — 거래완료/낙찰 포기/상품 내리기 확인 다이얼로그+상태 시뮬레이션, **차순위 연쇄 이양 안내 UI**(로직 Phase 5), `rating-modal` 별점>0 검증·완료 화면·닫기 초기화·코멘트 연결
...
- **T053 세부 — 동시성 핵심**
  - [ ] 입찰: 현재가+증가폭 검증·본인 상품 차단·`current_price` 갱신을 **단일 트랜잭션/RPC(원자적)** 처리 (경합 시 최신 현재가 기준 재검증)
  - [ ] 즉시구매: 즉시구매가 도달 시 경매 즉시 종료 + 낙찰자 확정 + 거래/채팅 생성을 원자적으로 처리 (입찰과의 레이스 컨디션 방지)
  - [ ] 🔴 ISSUE-003: 증가폭 정책 미결정 → 상수 사용, 정책 확정 시 검증식만 교체
- **T054 세부 — 자동 종료 (🔴 ISSUE-008 의존)**
  - [ ] 실행 메커니즘 선택: Supabase Edge Function + `pg_cron` / 외부 스케줄러 / 요청 시 lazy 처리 (ISSUE-008 결정 선행)
  - [ ] 36시간 만료 시 최고가 자동 낙찰, 입찰 없으면 유찰, 낙찰 시 `transactions`/`chat_rooms` 생성
- **T055 세부 — 연쇄 이양 (🔴 ISSUE-004/007 의존)**
  - [ ] 낙찰 포기 시 차순위 입찰자에게 **그의 입찰가**로 연쇄 이양, 입찰자 소진 시 유찰
  - [ ] 포기자 `penalties` 기록 (🔴 ISSUE-004: 제재 강도/누적 기준 미결정 → 기록만 우선)
  - [ ] 🔴 ISSUE-007: 차순위 수락 대기시간 적용 여부 미결정 → 우선 즉시 이양
- **T057 세부 — 실시간 채팅**
  - [ ] `messages` Realtime 구독, 낙찰 시 자동 생성된 방만 접근(RLS), 메시지 저장·정렬, 상대 평점 표시
- **T058 세부 — 거래완료/자동완료 (🔴 ISSUE-002/008 의존)**
  - [ ] 구매자 거래완료 확정 → `transactions.status=completed`
  - [ ] 미클릭 시 대기시간 경과 후 `auto_completed` (대기값 🔴 ISSUE-002 미결정 → 상수, 실행은 T054 메커니즘 재사용)
...
| T071 | - [ ] | 검색·필터·정렬(키워드/카테고리/지역/가격/상태)             | MVP 이후                         |
| T072 | - [ ] | 관심목록(찜)                                               | MVP 이후                         |
| T073 | - [ ] | 풀 마이페이지 대시보드(판매/구매 통계, 패널티 현황)        | MVP 이후                         |
| T074 | - [ ] | 관리자/신고·제재, 다국어(i18n: ko/en/ja)                   | MVP 이후 (가이드 i18n 전략 참조) |
| T075 | - [ ] | 미결정 정책 DB 이관 (경매시간/자동완료/증가폭/패널티/레벨) | ISSUE-001~007 확정 후            |
...
| ----- | ---- | -------------------- | ----------------------------------------------- |
| 001   | 🟡   | T011, T024, T052     | 36h 상수 사용, 추후 DB 이관(T075)               |
| 002   | 🔴   | T058, T063           | 자동완료 대기 임시 상수, 결정 후 DB화           |
| 003   | 🔴   | T023, T031, T053     | 입찰 증가폭 임시 정액, 검증식만 교체 가능 설계  |
| 004   | 🔴   | T055, T061           | 포기자 패널티 기록만 우선, 제재 강도 추후 결정  |
| 005   | 🔴   | T043, T059           | 레벨 산정 임시식, 가중치 조정 가능 구조         |
| 006   | 🔴   | T056                 | 입찰 후 내리기 제한 강도 미결정 → 보수적 차단   |
| 007   | 🔴   | T055                 | 차순위 수락 대기 미적용(즉시 이양)으로 우선     |
| 008   | 🔴   | **T054, T058, T063** | **자동 종료/완료 실행 메커니즘 결정 선행 필수** |

> ⚠️ **최대 리스크**: ISSUE-008(자동 종료/완료 실행 메커니즘)은 T054·T058의 **선행 결정 사항**입니다. Phase 5 진입 전 메커니즘(Edge Function+pg_cron / 외부 스케줄러 / lazy)을 확정하세요.

...
| ISSUE-030 | 🟢 DONE | 보안   | `products` RLS 컬럼 단위 보호 공백 → 경매 공정성 우회 가능 (최우선)         | 2026-07-17 마이그레이션 적용. BEFORE UPDATE 가드 트리거(description/buy_now_price만 허용, RPC는 우회 플래그). 롤백 스크립트 supabase/rollback/ |
| ISSUE-031 | 🟢 DONE | 보안   | 관리자 정지(user_suspensions)가 입찰/구매/철회/등록에서 미검사              | 2026-07-17 적용. assert_not_suspended() + RPC 4종/등록 트리거 검사. anon/authenticated 실행권한 회수                                           |
| ISSUE-032 | 🟢 DONE | 보안   | `submitReport` 원본 Postgres 에러 메시지 노출 + 대상/URL 검증 공백          | S3 코드 반영(고정 한글 메시지). S4·S5는 오적용 위험으로 보류(마이그레이션 주석 제안만)                                                         |
| ISSUE-033 | 🟢 DONE | 성능   | 경매 상세 페이지 데이터 페칭 워터폴(직렬 await) → 병렬화                    | Promise.all 병렬화 + getCurrentUserId react cache() + fetchAuctionDetail 5단→2단. P1~P3                                                        |
| ISSUE-034 | 🟢 DONE | 성능   | 이미지 과다조회·created_at 인덱스·업로드 병렬화 (P4는 보류)                 | 대표이미지 1장 조회·업로드 병렬화 코드 반영. 인덱스 2종 2026-07-17 적용. P4(RSC 번들)는 실효 없어 보류                                         |
| ISSUE-035 | 🟢 DONE | 스타일 | `auction-card` 즉시구매가 라이트모드 저대비(사실상 안 보임) — 실질 버그     | text-foreground로 교체(상세 페이지와 토큰 일치). Y1                                                                                            |
| ISSUE-036 | 🟢 DONE | 스타일 | 폼/쿼리 대규모 중복 리팩터링(이미지 슬롯·상태값 목록)                       | Y2 공용 ImageSlotGrid 추출(Playwright 검증) + Y7 AUCTION_STATUS_VALUES 단일 소스                                                               |
| ISSUE-037 | 🟢 DONE | 스타일 | 접근성·일관성 정리(aria-label·shadcn 컨벤션·cn·명명·타입가드 등)            | Y3~Y6 + 낮음 다수 반영                                                                                                                         |
...
## ISSUE-029 · 동시 입찰 동시성 검증 테스트(동일/다른 금액) 🟢 DONE(재현 스크립트 추가)

- **배경**: "동시에 같은 제품·같은 금액으로 입찰 시도 시 처리 로직"을 확인 요청받음. 조사 결과 **로직은 이미 올바르게 구현**되어 있어 수정은 불필요했고, 그 불변식을 재현 가능한 스크립트로 검증하기로 함(동일 금액 → 다른 금액 시나리오로 확장).
- **처리 로직(확인)**: 입찰은 원격 Supabase RPC `place_bid`가 원자적으로 처리한다. `SELECT * FROM products WHERE id = ? FOR UPDATE`로 **상품 행에 비관적 락**을 걸어 동일 상품 입찰을 직렬화하고, 락 획득 후 `p_amount >= current_price + min_bid_increment(codes.policy, 기본 1000)`으로 최소가를 **재검증**한다. 저장소의 `lib/mutations/auctions.ts::placeBid`·`components/auctions/bid-panel.tsx`는 얇은 호출/UX 사전검증 래퍼일 뿐이다(최종 판정은 서버 RPC). `bids`에는 amount unique 제약이 없으며 방어는 전적으로 이 **락 + 재검증** 조합에 의존한다.
- **동일 금액 시나리오**: 두 세션이 같은 금액 X로 동시 입찰 → 먼저 락을 잡은 1건만 성공(현재가=X 갱신) → 뒤 트랜잭션은 락 해제 후 **갱신된 현재가(=X)를 다시 읽어** `X < X + 1000`이므로 `최소 입찰가 …원 이상으로 입찰해 주세요.` 예외로 거부된다. 즉 **정확히 1건만 성공**(이중 낙찰·갱신 손실·데드락 없음).
- **다른 금액 시나리오**: 두 세션이 서로 다른 금액을 동시 입찰하면 락 획득 순서에 따라 결과가 갈리지만, `FOR UPDATE` 락이 상품 행에 걸려 직렬화되므로 아래 불변식은 **순서와 무관하게 항상 성립**한다.
  - 성공한 입찰들은 "이전 현재가 + 증가폭 이상" 조건으로 **엄격히 증가하는 체인**을 이룬다 → **최종 현재가 == 성공 입찰 중 최댓값**(낮은 입찰이 높은 입찰을 덮어쓰는 **갱신 손실 없음**).
  - 뒤로 밀린 입찰은 반드시 `최소 입찰가 …원 이상…` 사유로 거부되며, DB **입찰 건수 == 성공 건수**(거부 건은 미삽입).
- **추가한 검증 도구**:
  - `scripts/verify-concurrent-bid.mjs`(`test-smtp.mjs`의 `loadEnv(.env.local)` 패턴 재사용) — 두 개의 독립 인증 클라이언트가 `Promise.allSettled`로 같은 상품에 `place_bid`를 **동시 발사**하고, 이후 `products.current_price`·`bids` 건수를 **자체 read-back**(SELECT public)으로 확인해 불변식을 단언(PASS→exit 0 / 이중낙찰 등→FAIL exit 1). 인자로 시나리오 전환: `<productId> <amount>`(동일 금액) / `<productId> <amount1> <amount2>`(다른 금액). FOR UPDATE 락은 상품 행에 걸리므로 두 입찰자가 다른 사용자든 같은 사용자 2세션이든 검증 메커니즘은 동일(테스트 계정 2개 제약상 seller=A, 입찰자=B의 2세션).
  - `package.json`에 `test:bid-concurrency` 스크립트 추가.
- **검증(end-to-end, 2026-07-16)**: MCP로 활성 테스트 상품(seller A, 현재가 10000) 생성 후 실행. teardown은 `bids`/`products`에 **DELETE RLS 정책이 없어** MCP(RLS 우회)로 수행. `prettier`/`eslint` 통과.
  - **동일 금액**(`… <id> 11000`): 세션1 성공(11000) / 세션2 거부("최소 입찰가 12000원 이상…") → PASS. DB `bids=1`·`current_price=11000`.
